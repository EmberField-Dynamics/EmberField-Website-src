import { useState } from 'react'
import { Link } from 'react-router-dom'
import { useLanguage } from '../context/LanguageContext'
import { useTheme } from '../context/ThemeContext'
import { supabase, isSupabaseConfigured } from '../lib/supabase'

const SOCIAL_LINKS = [
  {
    label: 'Discord',
    href: 'https://discord.gg/emberfield',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="#94A3B8">
        <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
      </svg>
    ),
  },
  {
    label: 'GitHub',
    href: 'https://github.com/EmberField-Dynamics',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="#94A3B8">
        <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/>
      </svg>
    ),
  },
  {
    label: 'X (Twitter)',
    href: 'https://x.com/EmberfieldDynamics',
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="#94A3B8">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
      </svg>
    ),
  },
]

export default function Footer() {
  const { lang, t } = useLanguage()
  const { theme } = useTheme()
  const [email, setEmail] = useState('')
  const [subscribed, setSubscribed] = useState(false)
  const [saving, setSaving] = useState(false)
  const [newsletterError, setNewsletterError] = useState('')

  const p = (path) => `/${lang}${path}`
  const logoSrc = theme === 'dark' ? '/white-color-logo.png' : '/gray-color-logo.png'
  const serviceLinks = (t.nav.serviceOptions || []).slice(0, 4)

  const handleSubscribe = async (e) => {
    e.preventDefault()
    if (!email) return
    setSaving(true)
    setNewsletterError('')
    if (isSupabaseConfigured()) {
      const { error } = await supabase
        .from('newsletter_subscriptions')
        .insert({ email })
      if (error) {
        if (error.code !== '23505') {
          setNewsletterError(error.message)
          setSaving(false)
          return
        }
      }
    }
    setSubscribed(true)
    setSaving(false)
    setEmail('')
  }

  return (
    <footer style={{ background: '#030712', color: '#94A3B8', position: 'relative', overflow: 'hidden' }}>
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: "url('/worldmap.png')",
        backgroundSize: 'cover',
        backgroundPosition: 'center',
        opacity: 0.06,
        pointerEvents: 'none',
      }}/>

      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.12) 1px, transparent 1px)',
        backgroundSize: '24px 24px',
        opacity: 0.3,
        pointerEvents: 'none',
      }}/>

      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', padding: '80px 40px 0' }}>
        <div style={{
          background: 'rgba(16,185,129,0.06)',
          border: '1px solid rgba(16,185,129,0.15)',
          borderRadius: 0, padding: '40px',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          flexWrap: 'wrap', gap: '24px', marginBottom: '64px',
        }}>
          <div>
            <h3 style={{ color: '#fff', fontSize: '20px', fontWeight: 700, marginBottom: '8px' }}>{t.footer.stayUpdated}</h3>
            <p style={{ fontSize: '14px', color: '#94A3B8' }}>{t.footer.newsletterDesc}</p>
          </div>
          <form onSubmit={handleSubscribe} style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
            <input
              type="email" placeholder={t.footer.emailPlaceholder} value={email} onChange={e => setEmail(e.target.value)}
              className="field"
              style={{ width: '260px', height: '48px', fontSize: '14px', background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.1)', color: '#fff' }}
            />
            <button type="submit" className="btn-primary btn-slim" style={{ height: '48px', padding: '0 24px', opacity: saving ? 0.7 : 1 }}
              disabled={saving}>
              {saving ? '...' : subscribed ? t.footer.subscribed : t.footer.subscribe}
            </button>
            {newsletterError && (
              <span style={{ fontSize: '12px', color: '#F87171', width: '100%' }}>{newsletterError}</span>
            )}
          </form>
        </div>

        <div style={{
          display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr',
          gap: '48px', marginBottom: '64px',
        }} className="footer-grid">

          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              <img src={logoSrc} alt="Logo" style={{ height: '32px', objectFit: 'contain' }}/>
              <span style={{ color: '#fff', fontSize: '16px', fontWeight: 700 }}>Emberfield Dynamics</span>
            </div>
            <p style={{ fontSize: '14px', lineHeight: 1.8, maxWidth: '380px', color: '#94A3B8' }}>
              {t.footer.tagline}
            </p>
          </div>

          <div>
            <div className="footer-heading">{t.footer.nav}</div>
            {[
              [p('/'), t.footer.home],
              [p('/services'), t.footer.services],
              [p('/blog'), t.footer.blog],
              [p('/faq'), t.footer.faq],
              [p('/status'), t.footer.status],
              [p('/developers'), t.footer.developers],
              [p('/docs'), t.footer.docs],
              [p('/forums'), t.footer.forums],
              [p('/support/new'), t.footer.contact],
            ].map(([to, label]) => (
              <Link key={to} to={to} className="footer-link">{label}</Link>
            ))}
          </div>

          <div>
            <div className="footer-heading">{t.footer.services}</div>
            {serviceLinks.map((opt) => (
              <Link key={opt.id} to={opt.id ? `${p('/services')}/${opt.id}` : p('/services')} className="footer-link">
                {opt.title}
              </Link>
            ))}
          </div>

          <div>
            <div className="footer-heading">{t.footer.legal}</div>
            {[
              { label: t.footer.privacy, to: p('/privacy') },
              { label: t.footer.terms, to: p('/terms') },
              { label: t.footer.cookies, to: p('/cookies') },
            ].map((item) => (
              <Link key={item.to} to={item.to} className="footer-link">{item.label}</Link>
            ))}
          </div>
        </div>

        <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: '32px', paddingBottom: '32px' }}>
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            flexWrap: 'wrap', gap: '24px',
          }} className="footer-bottom">
            <div>
              <div style={{ fontSize: '13px', color: '#6B7280' }}>{t.footer.rights}</div>
              <div style={{ fontSize: '12px', color: '#4B5563', marginTop: '4px' }}>{t.footer.builtWith}</div>
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              {SOCIAL_LINKS.map((link) => (
                <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" className="social-btn" aria-label={link.label}>
                  {link.icon}
                </a>
              ))}
            </div>
          </div>
        </div>
      </div>

      <style>{`
        @media (max-width: 768px) {
          .footer-grid { grid-template-columns: 1fr 1fr !important; gap: 32px !important; }
          .footer-bottom { flex-direction: column !important; align-items: center !important; text-align: center !important; }
        }
        @media (max-width: 480px) {
          .footer-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </footer>
  )
}