  ...
  return (
    <a
      ...
      style={{
        ...
        transition: 'all 0.3s', display: 'block', animation: 'fadeInUp 0.6s ease-out both',
      }}
      onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--primary)'; e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 8px 40px rgba(16,185,129,0.1)' }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'none' }}
    >
...
export default function Developers() {
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '80px 24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '64px', animation: 'fadeInUp 0.6s ease-out' }}>
      ...
      {roles.map(role => {
        ...
        return (
          <div key={role.id} style={{ marginBottom: '64px', animation: 'fadeInUp 0.6s ease-out' }}>
            <h2 style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
              {roleMembers.map((m, i) => <MemberCard key={m.id} member={m}/>)}
            </div>
          </div>
        )
      })}
      ...
      {unassigned.length > 0 && (
        <div style={{ marginBottom: '64px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
            {unassigned.map((m, i) => <MemberCard key={m.id} member={m}/>)}
          </div>
        </div>
      )}

      {projects.length > 0 && (
        <div style={{ marginTop: '64px', animation: 'fadeInUp 0.6s ease-out' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 800, letterSpacing: '-0.5px' }}>Projects</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
            {projects.map((p, i) => <ProjectCard key={p.id} project={p}/>)}
          </div>
        </div>
      )}
    </div>
